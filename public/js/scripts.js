...
let isTransitioning = false; let currentSection = 'mainMenu';
...
var slides = document.querySelectorAll('.content-section');
...
function openMenu(){menuOpen=true;menuOverlay.classList.add('open');menuPanel.classList.add('open');}
function closeMenu(){menuOpen=false;menuOverlay.classList.remove('open');menuPanel.classList.remove('open');}
...
slides.forEach(function (s, i) {
   ...
   item.dataset.idx = i;
   item.innerHTML = '<span class="mi-num">' + s.dataset.abbr + '</span>' +
...
function refreshMenuActive() {
   mainMenuItems.forEach(function (el, i) { el.classList.toggle('current', i === idx) });
}
refreshMenuActive();
...
function getMyAge() {
   let today = new Date(),
      years = today.getFullYear() - 1983,
      months = today.getMonth() - 8;
   
   if(months < 0 || (months == 0) && today.getDate() < 19)
      years--;

   return years;
}

document.getElementById('age').innerHTML = (getMyAge()).toString();